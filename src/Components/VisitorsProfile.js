	...
	return (
		...
		/// image //*
			...
			<div class="row">
				...
				<div class="staff-id" style={{marginTop:'20px'}}>Card ID : <a href="#">{datas.ControlNo}</a></div>													
				<div class="staff-msg">
  ...
  const getColumnSearchProps = (dataIndex) => ({
	...
	useEffect(() => {
	  ...
	  const loadData = async () => {
		...
		let result  = await fetch("http://localhost:56630/api/Transactions/VisitorTransaction",{
			method: 'POST',
			headers: {
				"Content-Type": "application/json",
				"Accept": 'application/json'
			},
			body: JSON.stringify({Name: stdid[0].LastName + "," + stdid[0].FirstName + " " + stdid[0].MiddleName})
			})
						...
						<div class="column green1">
							<Form form={form} onFinish={handleCancel} >
								<Space style={{ display: "flex" }} >
									...
									<Form.Item style={{ marginBottom: '0px', marginTop:'-20px'}}>Diploma</Form.Item>
								</Space>
